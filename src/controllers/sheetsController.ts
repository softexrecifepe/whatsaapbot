...
export async function getSheetData(req: Request, res: Response) {
  try {
    ...
    if (Object.values(columnIndices).includes(-1)) {
      return res
        .status(400)
        .json({ message: "One or more required columns not found." });
    }
    ...
    const filteredData = rows
      ...
      .filter(
        (row: { Alunos: any; Turma: any; CPF: any; F: any }) =>
          row.Alunos && row.Turma && row.CPF && row.F
      ); 
    ...
    res
      .status(500)
      .json({ error: "An error occurred while retrieving the sheet data." });
  }
}
